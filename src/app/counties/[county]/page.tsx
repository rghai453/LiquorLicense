  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <nav className="mb-6 flex items-center gap-1.5 text-sm text-stone-400">
        ...
        <ChevronRight className="size-3" />
        <span className="font-medium text-stone-700">{countyName} County</span>
      </nav>
      ...
      <div className="mb-8 grid grid-cols-2 gap-px overflow-hidden rounded-xl bg-stone-200/80 ring-1 ring-stone-200">
        ...
        <div className="bg-white px-5 py-4">
          <p className="text-[11px] font-medium uppercase tracking-wider text-stone-400">Cities</p>
          <p className="text-2xl font-bold tabular-nums text-stone-900">{cityBreakdown.length}</p>
        </div>
      </div>

      <div className="mb-10">
        <h2 className="text-lg font-semibold text-stone-900 mb-4">Cities in {countyName} County</h2>
      ...
      <AdSlot slot="county-mid" format="horizontal" className="mb-10" />
          ...
          <Link key={lic.id} href={`/licenses/${lic.slug}`} className="group">
            <div className="h-full rounded-xl border border-stone-200/80 bg-white p-4 transition-all duration-200 hover:border-amber-300/60 hover:shadow-md hover:shadow-amber-900/5">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                {lic.city && <span className="text-xs text-stone-400">{lic.city}</span>}
              </div>
            </div>
          </Link>
        ))}
      ...
      {total > 24 && (
        <div className="text-center">
          <Link
            href={`/directory?county=${encodeURIComponent(countyName)}`}
